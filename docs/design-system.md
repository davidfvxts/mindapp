...
**Anatomy** (generated by `lib/stoneGeometry.ts` — deterministic SVG polygons,
one shared gradient per stone):
- **Table** — the flat top, always the first facet cut (Night 1 of a span)
- **Crown** — the ring(s) between table and girdle; finer rows on later stones
- **Pavilion** — the fan converging to the culet; the last band meets the point
- **Girdle** — the horizontal seam, stroked `--gem-girdle` (achromatic white)
- **Culet** — the bottom point (drifts off-centre on Aurora — the pear)
